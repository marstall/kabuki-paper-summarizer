...
export default class ClaimsGenerator extends LlmGenerator {
    async generate(params) {
        const ret = USE_PARAGRAPH_BASED_CLAIMS ? await this.generatePerParagraph(params)
            : await this.generatePerArticle(params)
        console.log({generateRet: ret})
        return ret;
    }

    async generatePerArticle(params) {
        const {articleId} = params
        console.log("generatePerArticle start")
        if (!articleId) throw ("articleId required.")
        const article = await loadArticle(articleId)
        const fullText = await extractFullTextFromArticle(article)
        const ret = await this.llm.chat(prompt, fullText, {
            stream: params.stream,
            max_tokens: 20000
        })
        console.log({generatePerArticle: ret})
        return ret;
    }

    async generatePerParagraph(params) {
        const {articleId} = params
        if (!articleId) throw ("articleId required.")
        const article = await loadArticle(articleId)
        let paragraphs = [];
