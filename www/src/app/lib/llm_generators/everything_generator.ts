...
async function generateHeadlines(llmName, {translationId}) {
...
async function saveHeadlines(json,llmName,{translationId}) {
  const translation = await prisma.translations.findUnique({
    where:{id:translationId},
    include: {articles:true}
    })
  // console.log({updating:translation.articles.id})
  // console.log({json})
  ...
  // log("updated w response",response)
}
...
export async function generateElement(elementName,llmName,params) {
  ...
  await save(JSON.parse(response.answer),llmName,params)
}
