...
import ChatExchangePanel
    from "@/app/components/chat-exchange-panel/chat-exchange-panel";
...
export default function Attachment({
    ...
    useEffect(() => {
        showCaption && fetch(`/api/attachment-translation/${attachment.id}`).then((response) => {
    ...
    const hoverTextOptions = [
        "click to show full plain-english caption",
        "click to show original caption", "click to return to plain-english caption."
    ]

    return <div
        className={maximized ? styles.containerMaximized : styles.containerInline}
        style={!reallyShowCaption ? {backgroundColor: 'white'} : {}}
        key={attachment.id}>
        <div onClick={toggleMaximized} className={styles.imageContainer}>
            {allowEdit ?
                <Link href={`/attachments/${attachment.id}`} className="button">
                    <Image
                        alt={attachment.alt_text || attachment.caption || 'Article attachment'}
                        src={url}
                ...
                : attachment.type === 'component' ?
                    ...
                    <div>
                        <Image
                            alt={attachment.alt_text || attachment.caption || 'Article attachment'}
                            loading="eager"
                            className={styles.image} src={url}
                            width={attachment.width || 600}
                            height={attachment.height || 1200}
                        /></div>
        ...
        {reallyShowCaption &&
            <div title={hoverTextOptions[captionState]} onClick={stepCaption}
                ...
                {captionStates[captionState] === "original" &&
                    <div><b>original caption</b></div>}
